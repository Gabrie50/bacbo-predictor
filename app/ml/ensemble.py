...
class AgenteEspecialista:
    ...
    def __init__(self, nome: str, padrao_nome: str, peso_inicial: float = 0.5, geracao: int = 0):
        ...
        print(
            f"   🧬 NOVO ESPECIALISTA: {nome} "
            f"(padrão: {padrao_nome}, peso: {peso_inicial:.2f}, geração: {geracao})"
        )
    ...
    def registrar_resultado(self, acertou: bool, resultado_real: Optional[str] = None):
        """Registra resultado e ajusta peso."""
        if acertou:
            self.acertos += 1
            self.peso_aprendido = min(2.5, self.peso_aprendido * 1.03)
            if resultado_real:
                self.dna['tendencia'] = max(
                    -0.3,
                    min(
                        0.3,
...
class EnsembleEvolutivo:
    ...
    def __init__(
        self,
        arquivo_estado: str = 'ensemble_evolutivo.pkl',
        ...
        print(
            f"   📈 Precisão atual: "
            f"{(self.acertos / self.total_previsoes * 100) if self.total_previsoes > 0 else 0:.1f}%"
        )
    ...
    def _criar_especialista_por_crossover(self):
        ...
        melhores = sorted(
            [agente for agente in self.agentes.values() if agente.total_usos > 20],
            key=lambda item: item.precisao,
            reverse=True
        )[:5]
        ...
        mae = random.choice([agente for agente in melhores[:3] if agente.nome != pai.nome])
    ...
    def _criar_especialista_por_mutacao(self):
        ...
        bons = [agente for agente in self.agentes.values() if agente.precisao > 0.6 and agente.total_usos > 30]
        if not bons:
            return None
    ...
    def _limpar_agentes_fracos(self):
        ...
        agentes_fracos.sort(key=lambda item: item[1])
    ...
    def prever(self, historico: List[dict]) -> dict:
        ...
        nomes_padroes_detectados = [padrao['nome'] for padrao in padroes_detectados]
        for agente in self.agentes.values():
            if agente.padrao_nome not in nomes_padroes_detectados and agente.total_usos > 10:
        ...
        estrategias = []
        for voto in votos_detalhados[:10]:
            if voto['agente'] != 'MEMORIA':
                estrategias.append(f"{voto['padrao']}({voto['confianca']:.0f}%)")
    ...
    def calcular_precisao_media(self) -> float:
        if not self.agentes:
            return 0
        precisoes = [agente.precisao for agente in self.agentes.values() if agente.total_usos > 10]
        return float(np.mean(precisoes)) if precisoes else 0

    def calcular_melhor_precisao(self) -> float:
        if not self.agentes:
            return 0
        return max([agente.precisao for agente in self.agentes.values() if agente.total_usos > 10] or [0])
