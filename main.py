# main.py - VERSÃO EVOLUTIVA COM CRIAÇÃO DINÂMICA DE ESPECIALISTAS
# =============================================================================
...
def health_urgente():
    return jsonify({
        ...
        'total_agentes': cache['ensemble'].get_stats()['total_agentes'] if cache.get('ensemble') else 0
    })
...
def salvar_previsao(previsao, resultado_real, acertou, contexto, total_agentes, geracao):
    ...
    try:
        ...
        } for r in contexto[:10]]) if contexto else None
        
        cur.execute('''
            INSERT INTO historico_previsoes 
            (data_hora, previsao, simbolo, confianca, resultado_real, acertou, 
             estrategias, modo, contexto_json, total_agentes, geracao)
...
# =============================================================================
# PROCESSADOR DE FILA COM ENSEMBLE EVOLUTIVO
# =============================================================================
                    ...
                    if salvar_rodada(rodada, 'principal'):
                        ...
                        if ultima_previsao_feita:
                            ...
                            if resultado_real != 'TIE':
                                ...
                                print(f"   Previsão: {ultima_previsao_feita['previsao']} | Real: {resultado_real}")
                                ...
                                # ENSEMBLE EVOLUTIVO APRENDE!
                                if cache.get('ensemble'):
                                    cache['ensemble'].aprender(resultado_real)
                                    print(f"   🧠 Ensemble aprendendo com resultado: {resultado_real}")
                                ...
                                print(f"👥 Total agentes: {cache['ensemble'].get_stats()['total_agentes']}")
                            
                            ultima_previsao_feita = None
                        
                        # =====================================================
                        # FAZER NOVA PREVISÃO COM ENSEMBLE EVOLUTIVO
                        # =====================================================
                        if len(historico_buffer) >= 30 and cache.get('ensemble') and ultima_previsao_feita is None:
                            print(f"\n🔮 FAZENDO PREVISÃO COM ENSEMBLE EVOLUTIVO...")
                            ...
                            if previsao['previsao'] != 'AGUARDANDO':
                                ...
                                print(f"   ✅ ENSEMBLE: {ultima_previsao_feita['previsao']} com {ultima_previsao_feita['confianca']}%")
                                print(f"   📊 Estratégias: {ultima_previsao_feita['estrategias'][:3]}")
                                print(f"   👥 Total agentes: {ultima_previsao_feita['total_agentes']}")
...
@app.route('/api/stats')
def api_stats():
    """API principal com estatísticas"""
    stats_ensemble = cache['ensemble'].get_stats() if cache.get('ensemble') else {}
    
    # Preparar especialistas para o frontend
    ...
    return jsonify({
        ...
        'fonte_ativa': 'ensemble_evolutivo',
        'estatisticas': {
...
def api_aprendizado():
    if cache.get('ensemble'):
        ...
        # Adicionar dados do mapa mental
...
def api_curto_prazo():
    ...
    return jsonify({
        ...
        'evolucao': {
            'total_agentes': stats.get('total_agentes', 0),
            'geracao': evolucao.get('geracao_atual', 0),
            'total_criados': evolucao.get('total_agentes_criados', 0)
        },
        'ultimos_ciclos': []
    })
...
def inicializar_sistema():
    ...
    # Thread para mostrar estatísticas periódicas
    def mostrar_estatisticas():
        while True:
            time.sleep(60)
            if cache.get('ensemble'):
                stats = cache['ensemble'].get_stats()
                print(f"\n📊 [STATUS] Agentes: {stats['total_agentes']} | Precisão: {stats['precisao']}% | Geração: {stats['evolucao']['geracao_atual']}")
    threading.Thread(target=mostrar_estatisticas, daemon=True).start()
...
if __name__ == "__main__":
    ...
    print("🎯 Sistema ENSEMBLE EVOLUTIVO ATIVO")
    print("🎯 Começa com 7 especialistas e CRIA NOVOS dinamicamente")
